...
impl VfsWrapper {
    ...
    pub fn create_dir_all(&self, path: impl Into<PathBuf>) {
        let path = path.into();
        let ancestors = path
            .iter()
    ...
    pub fn create_file(&self, path: impl Into<PathBuf>) -> FileWriter {
        let path = path.into();
    ...
    pub fn exists(&self, path: impl Into<PathBuf>) -> bool {
        self.fs.exists(path.into().to_str().unwrap()).unwrap()
    }
}
